...
public final class StrandOddsRatioUnitTest {
    ...
    @DataProvider(name = "dataForMap")
    private Object[][] dataForMap(){
        return new Object[][]{
                {new StrandOddsRatio(), GATKVCFConstants.STRAND_ODDS_RATIO_KEY},
                {new AS_StrandOddsRatio(), GATKVCFConstants.AS_STRAND_ODDS_RATIO_KEY},
        };
    }

    @Test(dataProvider = "dataForMap")
    public void testUsingMap(final InfoFieldAnnotation ann, final String key){
    ...
    public void testUsingMap_Raw(){
        ...
        final Map<String, Object> annotatedMap = ann.annotateRawData(null, vc, stratifiedPerReadAlleleLikelihoodMap);
        final String sorString = (String)annotatedMap.get(key);

        final String expectedString = AS_StrandBiasTest.rawValueAsString(table);

        Assert.assertEquals(sorString, expectedString);
    }
}
